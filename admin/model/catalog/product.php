...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
        ...
        if($_FILES['isbn']) {
            $isbnFile = $_FILES['isbn'];

            if (move_uploaded_file($isbnFile['tmp_name'], DIR_FILES . $isbnFile['name'])) {
                $data['isbn'] = $isbnFile['name'];
            } else {
                $data['isbn'] = '';
            }
        } else {
...
//				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "'");
    ...
    public function saveProduct($product_id, $data) {
        $sql = "UPDATE " . DB_PREFIX . "product SET " . ($data['price'] ? "price = '" . (float)$data['price'] . "', ": "") . "status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
    }
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', currency = '" . $this->db->escape($data['currency']) . (isset($data['isbn']) && $data['isbn'] ? "', isbn = '" . $this->db->escape($data['isbn']) : "") . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', category_id = '" . (int)$data['category2_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
