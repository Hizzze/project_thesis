...
class ModelToolExportImport extends Model {
    ...
    protected function storeCategoryIntoDatabase( &$category, &$languages, $exist_meta_title, &$layout_ids, &$available_store_ids, &$url_alias_ids ) {
        ...
        $meta_titles = $category['meta_titles']; //added
        ...
        foreach ($languages as $language) {
            ...
            $meta_title = isset($meta_titles[$language_code]) ? $this->db->escape($meta_titles[$language_code]) : ''; //added
            ...
            if ($exist_meta_title) {
                ...
                $sql .= "( $category_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword' );";
            } else {
                $sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_title`, `meta_keyword`) VALUES ";
                $sql .= "( $category_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword' );";
            }
    ...
    protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
        ...
        for ($i=0; $i<$k; $i+=1) {
            ...
            $category['meta_titles'] = $meta_titles; //added
...
//		$main_category = $product['main_category']; //added
        ...
        $currency_price = trim($product['currency_price']);
        ...
        $meta_titles = $product['meta_titles']; //added
        ...
        $sql .= "`location`,`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`currency_price`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
        $sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`,`currency`) VALUES ";
        $sql .= "($product_id,$quantity,'$sku','$upc',";
        $sql .= in_array('ean',$product_fields) ? "'$ean'," : "";
        $sql .= in_array('jan',$product_fields) ? "'$jan'," : "";
        $sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
        $sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
        $sql .= "'$location',$stock_status_id,'$model',$manufacturer_id,'$image',$shipping,$price,$currency_price,$points,";
        $sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
        $sql .= ($date_modified=='NOW()') ? "$date_modified," : "'$date_modified',";
        $sql .= ($date_available=='NOW()') ? "$date_available," : "'$date_available',";
        $sql .= "$weight,$weight_class_id,$status,";
        $sql .= "$tax_class_id,$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum','$currency');";
        $this->db->query($sql);
        foreach ($languages as $language) {
            $language_code = $language['code'];
            $language_id = $language['language_id'];
            ...
            if ($exist_table_product_tag) {
                if ($exist_meta_title) {
                    $sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_title`, `meta_keyword`) VALUES ";
                    $sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword' );";
                } else {
                    $sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_title`, `meta_keyword`) VALUES ";
                    $sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword' );";
                }
                ...
                if ($exist_meta_title) {
                    $sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_title`, `meta_keyword`, `tag`) VALUES ";
                    $sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword', '$tag' );";
                } else {
                    $sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_title`, `meta_keyword`, `tag`) VALUES ";
                    $sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_title', '$meta_keyword', '$tag' );";
                }
...
//			$main_category = $this->getCell($data,$i,$j++,''); //added
            ...
            $currency = $this->getCell($data,$i,$j++);
            $price2 = ($currency && $this->currency->has($currency))? $this->currency->convert($price, null, $currency): $price;
            $product = array();
...
//			$product['main_category'] = $main_category; //added
            ...
            $product['meta_titles'] = $meta_titles; //added
    ...
    protected function getFilterGroupIds() {
        $language_id = $this->getDefaultLanguageId();
        $sql  = "SELECT option_id, name FROM `".DB_PREFIX."ocfilter_option_description` ";
        $sql .= "WHERE language_id='".(int)$language_id."'";
        $query = $this->db->query( $sql );
        $filter_group_ids = array();
        foreach ($query->rows as $row) {
            $filter_group_id = $row['option_id'];
            $name = $row['name'];
    ...
    protected function getFilterIds() {
        $language_id = $this->getDefaultLanguageId();
        $sql  = "SELECT f.option_id, fd.value_id, fd.name FROM `".DB_PREFIX."ocfilter_option_value_description` fd ";
        $sql .= "INNER JOIN `".DB_PREFIX."ocfilter_option_value` f ON f.value_id=fd.value_id ";
        $sql .= "WHERE fd.language_id='".(int)$language_id."'";
        $query = $this->db->query( $sql );
        $filter_ids = array();
        foreach ($query->rows as $row) {
            $filter_group_id = $row['option_id'];
            $filter_id = $row['value_id'];
            $name = $row['name'];
    ...
    protected function storeProductFilterIntoDatabase( &$product_filter, &$languages ) {
        ...
        $option_id = $product_filter['option_id'];
        $option_value_id = $product_filter['option_value_id'];
        $sql  = "INSERT INTO `".DB_PREFIX."ocfilter_option_value_to_product` (`product_id`, `option_id`, `value_id`) VALUES ";
        $sql .= "( $product_id, $option_id, $option_value_id );";
        $this->db->query( $sql );
    }

    protected function deleteProductFilters() {
        $sql = "TRUNCATE TABLE `".DB_PREFIX."ocfilter_option_value_to_product`";
        $this->db->query( $sql );
    }


    protected function deleteProductFilter( $product_id ) {
        $sql = "DELETE FROM `".DB_PREFIX."ocfilter_option_value_to_product` WHERE product_id='".(int)$product_id."'";
        $this->db->query( $sql );
    }
    ...
    protected function uploadProductFilters( &$reader, $incremental, &$available_product_ids ) {
        ...
        for ($i=0; $i<$k; $i+=1) {
            ...
            $product_filter['option_id'] = $filter_group_id;
            $product_filter['option_value_id'] = $filter_id;
            if (($incremental) && ($product_id != $previous_product_id)) {
    ...
    protected function validateCategories( &$reader ) {
        ...
        if ($exist_meta_title) {
            ...
            ( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_title", "meta_description", "meta_title", "meta_keywords", "store_ids", "layout", "status" );
            $expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_title", "meta_keywords" );
        } else {
            $expected_heading = array
            ( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_description", "meta_title", "meta_keywords", "store_ids", "layout", "status" );
            $expected_multilingual = array( "name", "description", "meta_description", "meta_title", "meta_keywords" );
        }
    ...
    protected function validateProducts( &$reader ) {
        ...
        $expected_heading = array_merge( $expected_heading, array( "meta_description", "meta_title", "meta_keywords", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum" ) );
        if ($exist_meta_title) {
            $expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_title", "meta_keywords", "tags" );
        } else {
            $expected_multilingual = array( "name", "description", "meta_description", "meta_title", "meta_keywords", "tags" );
        }
        return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
    }
    ...
    protected function validateFilterColumns( &$reader ) {
        ...
        $language_id = $this->getDefaultLanguageId();
        $sql  = "SELECT fgd.option_id, fgd.name AS option_name, fd.value_id, fd.name AS option_value_name ";
        $sql .= "FROM `".DB_PREFIX."ocfilter_option_description` fgd ";
        $sql .= "LEFT JOIN `".DB_PREFIX."ocfilter_option_value` f ON f.option_id=fgd.option_id ";
        $sql .= "LEFT JOIN `".DB_PREFIX."ocfilter_option_value_description` fd ON fd.value_id=f.value_id AND fd.language_id='".(int)$language_id."' ";
        ...
        foreach ($query->rows as $row) {
            if ($export_import_settings_use_filter_group_id) {
                ...
                if ($export_import_settings_use_filter_id) {
                    ...
                    $filter_name = htmlspecialchars_decode(trim($row['option_value_name']));
                    if (!is_null($filter_name)) {
                        $filter_groups[$filter_group_id][$filter_name] = true;
                    }
                }
            } else {
                $filter_group_name = htmlspecialchars_decode(trim($row['option_name']));
...
//		}
        ...
        foreach ($languages as $language) {
            ...
            foreach ($results->rows as $key=>$row) {
                if (isset($category_descriptions[$language_code][$key])) {
                    ...
                    $results->rows[$key]['meta_title'][$language_code] = $category_descriptions[$language_code][$key]['meta_title']; //added
                    ...
                    $results->rows[$key]['meta_title'][$language_code] = '';//added
    ...
    protected function getFilterGroupNames( $language_id ) {
        ...
        $sql .= "FROM `".DB_PREFIX."ocfilter_option_description` ";
        $sql .= "WHERE language_id='".(int)$language_id."' ";
    ...
    protected function getFilterNames( $language_id ) {
        $sql  = "SELECT value_id, name ";
        $sql .= "FROM `".DB_PREFIX."ocfilter_option_value_description` ";
        $sql .= "WHERE language_id='".(int)$language_id."' ";
        ...
        foreach ($languages as $language) {
            ...
            foreach ($results->rows as $key=>$row) {
                if (isset($product_descriptions[$language_code][$key])) {
                    ...
                    $results->rows[$key]['meta_title'][$language_code] = $product_descriptions[$language_code][$key]['meta_title']; //added
...
//		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('main_category'),4)+1); //added
        ...
        foreach ($languages as $language) { //added
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('meta_title')+4,32)+1);
        } //added
        foreach ($languages as $language) {
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('currency'),8)+1);
...
//			$data[$j++] = $main_category; //added
            ...
            $data[$j++] = ($row['currency'] && $this->currency->has($row['currency']))
                ? $this->currency->convert($row['price'], $row['currency'], null)
                : $row['price'];
            $data[$j++] = $row['points'];
            $data[$j++] = $row['date_added'];
            $data[$j++] = $row['date_modified'];
            ...
            $data[$j++] = $row['minimum'];
            $data[$j++] = isset($row['currency']) ? $row['currency'] : '';
            $this->setCellRow( $worksheet, $i, $data, $this->null_array, $styles );
            $i += 1;
            $j = 0;
        }
    ...
    protected function getProductFilters( $min_id, $max_id ) {
        $sql  = "SELECT ovf.product_id, fo.option_id, ovf.value_id ";
        $sql .= "FROM `".DB_PREFIX."ocfilter_option_value_to_product` ovf ";
        $sql .= "INNER JOIN `".DB_PREFIX."ocfilter_option_value` ov ON ov.value_id=ovf.value_id ";
        $sql .= "INNER JOIN `".DB_PREFIX."ocfilter_option` fo ON fo.option_id=ov.option_id ";
        if (isset($min_id) && isset($max_id)) {
            $sql .= "WHERE product_id BETWEEN $min_id AND $max_id ";
        }
        $sql .= "ORDER BY ovf.product_id ASC, fo.option_id ASC, ov.value_id ASC";
        $query = $this->db->query( $sql );
        $product_filters = array();
        foreach ($query->rows as $row) {
            $product_filter = array();
            $product_filter['product_id'] = $row['product_id'];
            $product_filter['option_id'] = $row['option_id'];
            $product_filter['value_id'] = $row['value_id'];
    ...
    protected function populateProductFiltersWorksheet( &$worksheet, &$languages, $default_language_id, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
        ...
        foreach ($product_filters as $row) {
            ...
            if ($this->config->get( 'export_import_settings_use_filter_group_id' )) {
                ...
                $data[$j++] = html_entity_decode($filter_group_names[$row['option_id']],ENT_QUOTES,'UTF-8');
            }
            if ($this->config->get( 'export_import_settings_use_filter_id' )) {
                $data[$j++] = $row['value_id'];
            } else {
                $data[$j++] = html_entity_decode($filter_names[$row['value_id']],ENT_QUOTES,'UTF-8');
            }
            $this->setCellRow( $worksheet, $i, $data, $this->null_array, $styles );
            $i += 1;
            $j = 0;
