...
$('#translation').on('click', '#button-save', function(e) {
	...
	$.ajax({
		url: 'index.php?route=design/language/save&token=<?php echo $token; ?>&store_id=' + $('select[name="store_id"]').val() + '&language_id=' + $('select[name="language_id"]').val() + '&path=' + $('input[name="path"]').val(),
		type: 'post',
		data: $("#translation input, #translation textarea").serializeArray(),
		dataType: 'json',
		beforeSend: function() {
			$(node).button('loading');
		},
