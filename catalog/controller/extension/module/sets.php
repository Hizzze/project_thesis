        ...
        if (!empty($product_info['sets'])) {
            ...
            foreach ($sets as $s_key => &$set) {
                ...
                foreach ($set['products'] as $pkey => &$product) {
                    ...
                    if ($pr_info['image']) {
                        ...
                        if (floatval(VERSION) >= 2.2)
                            $product['thumb'] = $this->model_tool_image->resize($pr_info['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
                        else
                ...
                if (isset($set['discount']))
                    ...
                    $set['discount'] = $this->currency->format($discount,$this->session->data['currency'],'',false);
                    if (floatval(VERSION) >= 2.2) {
