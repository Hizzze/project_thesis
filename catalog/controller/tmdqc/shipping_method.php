...
class ControllerTmdqcShippingMethod extends Controller
{
    public function index()
    {
            ...
            foreach ($results as $result) {
                ...
                if ($this->config->get($result['code'] . '_status')) {
                    ...
                    if (isset($quote['title'])) {
                        $title = $quote['title'];
                    } elseif (!empty($tmdshippingsetting[$this->config->get('config_language_id')]['label'])) {
        ...
        $data['text'] = [
            'np_city' => $this->language->get('np_city'),
            'np_warehouse' => $this->language->get('np_warehouse'),
            'np_choose_warehouse' => $this->language->get('np_choose_warehouse'),
            'start_typing' => $this->language->get('start_typing'),
        ];
    ...
    public function set()
    {
        ...
        $this->response->setOutput(json_encode($this->session->data['shipping_address']));
        $data = [
            'firstname' => ($this->request->post['firstname']) ? $this->request->post['firstname'] : '',
            'lastname' => ($this->request->post['lastname']) ? $this->request->post['lastname'] : '',
            'company' => ($this->request->post['company']) ? $this->request->post['company'] : '',
            'address_1' => ($this->request->post['address_1']) ? $this->request->post['address_1'] : '',
            'address_2' => ($this->request->post['address_2']) ? $this->request->post['address_2'] : '',
            'postcode' => ($this->request->post['postcode']) ? $this->request->post['postcode'] : '',
            'city' => ($this->request->post['city']) ? $this->request->post['city'] : '',
            'zone_id' => ($this->request->post['zone_id']) ? $this->request->post['zone_id'] : '',
            'country_id' => ($this->request->post['country_id']) ? $this->request->post['country_id'] : '',
    ...
    public function save()
    {
        ...
        if (!isset($this->request->post['shipping_method'])) {
            ...
            if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($this->session->data['shipping_methods'][$shipping[1]]['quote'][$shipping[0]])) {
                $json['error']['warning'] = $this->language->get('error_shipping');
            }
        }

        if (!$json) {
            $this->session->data['shipping_method'] = $this->session->data['shipping_methods'][$shipping[1]]['quote'][$shipping[0]];
